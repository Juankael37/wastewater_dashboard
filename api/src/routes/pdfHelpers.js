...
export function addFooters(pages, fonts, pageWidth, margin) {
  for (let i = 0; i < pages.length; i++) {
    ...
    p.drawText('Generated by Wil-C - Wastewater Monitoring System', { x: margin, y: 26, size: 7.5, font: fonts.reg, color: CLR.textMuted })
...
// ── Embed Wil-C logo from bundled file ─────────────────────────────────────
export async function embedLogo(pdfDoc, env = null) {
  try {
    const frontendUrl = env?.FRONTEND_URL || 'https://wastewater-dashboard.pages.dev'
    const logoUrl = `${frontendUrl}/Official%20Header%20logo.png`
    const imgData = await fetchImageBytes(logoUrl)
    if (!imgData) return null
    const format = getImageFormat(imgData.contentType, logoUrl)
    if (format === 'png') {
      const embeddedLogo = await pdfDoc.embedPng(imgData.bytes)
      console.log('[pdf-logo] Wil-C logo embedded successfully (PNG)')
      return embeddedLogo
    }
    if (format === 'jpg' || format === 'jpeg') {
      const embeddedLogo = await pdfDoc.embedJpg(imgData.bytes)
      console.log('[pdf-logo] Wil-C logo embedded successfully (JPG)')
      return embeddedLogo
    }
    console.warn(`[pdf-logo] Unsupported logo format: ${format}`)
    return null
  } catch (err) {
    console.error('[pdf-logo] Logo embed error:', err.message)
    return null
  }
}
